...
bool Matematyka::Funkcje::checkSum(const std::vector<int>& numbers, int sum) const {
    auto sorted_numbers = numbers;
    std::sort(sorted_numbers.begin(), sorted_numbers.end());
    for (auto number_it = sorted_numbers.cbegin(); number_it != sorted_numbers.cend();
         ++number_it) {
        ...
        for (auto second_number_it = number_it + 1;
             second_number_it != sorted_numbers.cend() && *second_number_it <= look_for;
             ++second_number_it) {
